...
class _ProfileadminState extends State<Profileadmin> {
  ...
  // void _updateFirestoreAfterEmailVerification(String newEmail) async {
  //   User? user = FirebaseAuth.instance.currentUser;
  //
  //   if (user != null && user.email == newEmail && user.emailVerified) {
  //     // Xác nhận rằng email mới đã được xác thực và giống với email hiện tại của người dùng
  //     await FirebaseFirestore.instance
  //         .collection('users')
  //         .doc(user.uid)
  //         .update({'email': newEmail});
  //     print('Đã xác thực email mới và cập nhật thông tin trong Firestore');
  //   } else {
  //     print('Email mới chưa được xác thực hoặc không khớp với email hiện tại');
  //   }
  // }
...
void updata(BuildContext context,Map<String, dynamic> userData) async {
  ...
  TextEditingController bcController =
  TextEditingController(text: userData['bangcap']);
  TextEditingController tenctController =
  TextEditingController(text: userData['namecongty']);
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: isLoading
                    ...
                    String newbc = bcController.text;
                    String newtenct =tenctController.text;
                    if (_imageFile != null) {
